...
class CrowdNavPyBulletEnv(gym.Env):
    def __init__(self, num_peds=0, max_static=8, max_steps=2000, resolution=0.1):
        super().__init__()
    ...
    def find_free_grid(self, label, avoid=None, min_dist=0.5, max_attempts=100):
        for _ in range(max_attempts):
            ...
            if avoid is None or np.linalg.norm(w - (avoid if isinstance(avoid,np.ndarray) else np.array(avoid))) >= min_dist:
                return (r, c)
    ...
    def reset(self, visualize=False):
        ...
        # fixed start/goal for repeatability
        ...
        if replanned:
            self._draw_astar_path(self.path.world_path(),
                                ...
                                label_every=1,  # or 2/3 if cluttered
                                text_size=1.2)
            
        # Prime HUD near robot
    ...
    def _update_pose(self):
        # If Dynamics.get_pose() returns wrapped θ in (-π,π], unwrap it to continuous
        pos, theta_wrapped = self.dyn.get_pose()
